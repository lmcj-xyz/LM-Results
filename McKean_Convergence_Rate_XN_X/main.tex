...
% to add Elena's comments faster
...
proposed in
\cite{issoglio_mckean_2021}. \ei{not yet for McKean SDEs, here you are only looking at SDEs with distributional coefficients in Besov spaces, SDEs framed as martingale problems}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{definition}
For any real-valued continuous semi-martingale \ei{you need to specify the name of this semimartingale, that is $\bar Y$}, the local time at zero
$L^{0}_{t} (\bar{Y})$
is defined as
\begin{equation}
...
\ei{Instead of this sentence you should write something like `The lemma below is from \cite{de_angelis_numerical_2020} and its
proof can be found in \cite[Lemma 5.1]{de_angelis_numerical_2020}. We include the statement for ease of reading.'}
...
\end{lemma}
\ei{It is confusing to call the semimartingale $\bar Y$ above and $Z$ in the lemma. It is not wrong, but not the best use of notation.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{definition}{Kolmogorov equations}
...
\begin{proof}
    ...
    Plugging
    \cref{eq:norm_bounded_by_rho_norm}
    into
    \cref{eq:u-uN_in_Linfinity_bounded_by_u-uN_in_Calpha1}
   ...
   For
   \cref{eq:graduNu_bounded_by_bNb}
   recall that if
   $f \in \mathcal{C}^{1 + \alpha}$
   then
...
for some
$k$, from \eqref{eq:local_time_diff}
we get
...
4 (1 + \lambda) \qty(
  ...
  +
	\mathbb{E}\qty[\int_0^t\abs{Y^N_s - Y^N_s} ds]
	)
	\\
&+
\frac{1}{\epsilon} 3 t \norm{ b^N - b}_{C_T \mathcal{C}^{-\beta}}
...
\end{split}
\end{equation}
 \ei{add explanation of why you use absolute value of difference of $u^N -u$, because in \eqref{eq:local_time_diff} there is no absolute value}
\ei{explain how you get 4 in front of $(1+\lambda)$ by bounding the indicator functions under the integral in \eqref{eq:local_time_diff}}
